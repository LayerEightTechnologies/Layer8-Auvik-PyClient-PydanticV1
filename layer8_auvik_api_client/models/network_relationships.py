...
class NetworkRelationships(BaseModel):
    """
    This network's relationships to other resources  # noqa: E501
    """
    network_detail: Optional[NetworkRelationshipsNetworkDetail] = Field(None, alias="networkDetail")
    tenant: Optional[Tenant] = None
    devices: Optional[NetworkRelationshipsDevices] = None
    __properties = ["networkDetail", "tenant", "devices"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NetworkRelationships:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> NetworkRelationships:
        """Create an instance of NetworkRelationships from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NetworkRelationships.parse_obj(obj)

        _obj = NetworkRelationships.parse_obj({
            "network_detail": NetworkRelationshipsNetworkDetail.from_dict(obj.get("networkDetail")) if obj.get("networkDetail") is not None else None,
            "tenant": Tenant.from_dict(obj.get("tenant")) if obj.get("tenant") is not None else None,
            "devices": NetworkRelationshipsDevices.from_dict(obj.get("devices")) if obj.get("devices") is not None else None
        })
        return _obj
